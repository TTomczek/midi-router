...
public sealed class MidiRouter : IDisposable
{
    ...
    public IReadOnlyDictionary<string, int> Assignments
    {
        get
        {
            lock (synchronization)
            {
                return new Dictionary<string, int>(allocator.Assignments, StringComparer.Ordinal);
            }
        }
    }
    ...
    public void Deactivate(string deviceId)
    {
        lock (synchronization)
        {
            ...
            allocator.Remove(deviceId);
            lastOriginalChannels.Remove(deviceId);
